#!/usr/bin/env python3
"""
Legal Document RAG Query System with Unstructured PDF Parser

An improved version of the Legal RAG system that uses the Unstructured library
for better PDF text extraction. This provides superior parsing quality for complex
legal documents compared to the basic PyMuPDF parser.

Key improvements over the basic version:
- Uses Unstructured library for robust PDF parsing
- Better handling of complex legal document layouts
- Improved text extraction from tables and multi-column formats
- Local data storage in Daniel/data/ folder
...
class LegalRAGSystem:
    ...
    This version uses the Unstructured library for improved PDF parsing,
    which provides better text extraction from complex legal documents.
    """

    def __init__(
        self,
        documents_dir: str = "./data/legal_docs",
        index_dir: str = "./rag_index_unstructured",
        embedding_model: str = "BAAI/bge-small-en-v1.5",
        llm_model: str = "claude-3-5-sonnet-20241022",
        temperature: float = 0.7,
    ):
        """
        Initialize the Legal RAG System with Unstructured parser.

        Args:
            documents_dir: Path to directory containing PDF documents (default: ./data/legal_docs)
    ...
    def setup(self):
        ...
        print("=" * 70)
        print("🚀 Legal Document RAG System - Setup (Unstructured Parser)")
        print("=" * 70)
    ...
    def _create_index(self):
        """
        Create new index from PDF documents using Unstructured library.

        The Unstructured library provides superior PDF parsing compared to
        basic readers, with better handling of:
        ...
        if not self.documents_dir.exists():
            raise FileNotFoundError(
                ...
                f"Expected path: {self.documents_dir.absolute()}"
            )
        ...
        # Initialize UnstructuredReader for better PDF parsing
        print("⏳ Initializing Unstructured PDF parser...")
        ...
        for pdf_file in pdf_files:
            ...
            try:
                # Load with Unstructured - better text extraction
    ...
    def query(self, question: str, verbose: bool = True) -> str:
        ...
        Args:
            ...
            verbose: Whether to print the question and answer
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ["-h", "--help"]:
        print(__doc__)
        print("\nUsage:")
        print("  python legal_rag_query_unstructured.py           # Interactive mode")
        print("  python legal_rag_query_unstructured.py test      # Run test questions only")
        print("  python legal_rag_query_unstructured.py --help    # Show this help")
        print("\nFeatures:")
        print("  - Uses Unstructured library for superior PDF parsing")
        print("  - Better text extraction from complex legal documents")
        print("  - Handles multi-column layouts and tables effectively")
        print("  - Local data storage in ./data/legal_docs/")
        print("\nComparison with basic version:")
        print("  legal_rag_query.py             → Basic PyMuPDF parser")
        print("  legal_rag_query_unstructured.py → Advanced Unstructured parser (this file)")
        return
